	...
	public function checkStatus(Request $request)
	{
		...
		if($user->status=="Active") {
			...
			if(!$user->vehicles->count()){
				...
				$message = __('messages.user.car_details_message1');
			} elseif(!$vehicle_documents) {
				$user->status = "Car_details";
				$message = __('messages.user.car_details_message2');
			} elseif(!$driver_documents) {
				$user->status = "Document_details";
				$message = __('messages.user.document_details_message');
			} else {
				$message = __('messages.user.active_message');
			}

		} else if($user->status=="Pending") {
			$message = __('messages.user.pending_message');
			...
			$message = __('messages.user.inactive_message');
		}
