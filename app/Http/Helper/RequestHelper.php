...
class RequestHelper
{
						...
						if(@$check) {
					        ...
					        $push_data['data'] = array(
								'ride_request' => array(
									...
									'distance'          => $get_min_time['distance'],
	...
	public function trip_assign($array)
	{
		...
		$data = array('manual_booking_trip_assigned' => array(
			...
			'car_app_image' 	=> $ride_request->car_type->app_image,
	...
	public function SendPushNotification($user,$push_data)
	{
        ...
        try {
        	if ($device_type == 1) {
				...
				$firbase->sendAppleNotification($push_title, $data, $user_type, $device_id, $change_title,$firebase_data);
				logger("IOS Push");	
            }
            else {
				$firbase->sendAndroidNotification($push_title, $data, $user_type, $device_id, $change_title,$firebase_data);
				logger("Andriod Push");
            }
        }
        catch (\Exception $e) {
