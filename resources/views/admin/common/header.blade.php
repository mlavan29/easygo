            ...
            <ul class="nav navbar-nav">
                ...
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true"
                        aria-expanded="false">
                        <x-icon class="w-5 h-5" name="flag-language-{{ $default_language[0]->value }}" />
                        {{ $default_language[0]->name }}
                        <span class="caret"></span></a>
                    <ul class="dropdown-menu">
                        @foreach ($language as $key => $lang)
                            @if ($key != App::getLocale())
                                <li> <a class="dropdown-item inline" href="{{ route('lang.switch', $key) }}">
                                        <x-icon class="w-5 h-5" name="flag-language-{{ $key }}" />
                                        {{ $lang }}</a></li>
                            @endif
                        @endforeach
                    </ul>
                ...
                <li class="dropdown user user-menu">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                        ...
                        @if (!$company_user || $user->profile == null)
                            <img src="{{ url('admin_assets/dist/img/avatar04.png') }}" class="user-image"
                                alt="User Image">
                        @else
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li class="user-header">
                            ...
                            @if (!$company_user || $user->profile == null)
                                <img src="{{ url('admin_assets/dist/img/avatar04.png') }}" class="img-circle"
                                    alt="User Image">
                            @else
                        ...
                        <li class="user-footer">
                            ...
                            <div class="pull-right">
                                <a href="{{ url($company_user ? 'company/logout' : 'admin/logout') }}"
        ...
        @if ($company_user)
            <select id="js-currency-select" class="form-control" style="display: none;">
                @foreach ($currency_select as $code)
                    <option value="{{ $code }}" @if (session('currency') == $code) selected="selected" @endif>
                        {{ $code }}</option>
                @endforeach
            </select>
        @endif
